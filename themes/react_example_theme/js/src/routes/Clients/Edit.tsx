...
const EditClient: React.FC = () => {
  ...
  const fetchClient = async () => {
    try {
      ...
      const mappedClient: Client = {
        ...
        address: clientData.field_address.address_line1,
        city: clientData.field_address.locality,
        state: clientData.field_address.administrative_area,
        zip: clientData.field_address.postal_code, 
  ...
  const handleUpdate = async (values: ClientFormValues) => {
    ...
    if (clientId) {
      await createClient(updateClientData, form); 
    }
