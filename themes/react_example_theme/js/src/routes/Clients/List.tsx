...
import { Button, List, Space, Spin, Table } from 'antd';
...
const Clients: React.FC = () => {
    ...
    const fetchClients = async () => {
        try {
            const response = await fetch(`https://defender.ddev.site/jsonapi/node/clients?page[limit]=10`);
    ...
    console.log(clients, "I am clients")
    return (
        <List>
        {isLoading ? (
            ...
            <Table.Column title="Actions" dataIndex="actions" key="actions"
              render={(_, record: Client) => (
                <Space>
                    ...
                    <Link  to={`show/${record.uuid}`}>Show</Link>
                </Space>
              )} />
          </Table>
        )}
