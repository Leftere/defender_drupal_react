...
type UpdateAppointmentStatusHook = {
  updateAppointmentServiceStatus: (id: string, status: string, color: string) => Promise<void>;
  isLoading: boolean;
  error: string | null;
  csrfToken: string | null;
};

export const useUpdateAppointmentSerivceStatus = (): UpdateAppointmentStatusHook => {
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [csrfToken, setCsrfToken] = useState<string | null>(null); // State to store CSRF token

  const updateAppointmentServiceStatus = useCallback(async (id: string, status: string, color: string): Promise<void> => {
    setIsLoading(true);
    setError(null);
  ...
  return { updateAppointmentServiceStatus, isLoading, error, csrfToken };
};
